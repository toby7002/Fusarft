...
class FPlacedFeatures {
	companion object {
		val UPPER_HAFNIUM_ORE_PLACED_KEY: ResourceKey<PlacedFeature> = registerKey("upper_hafnium_ore_placed")
		val MIDDLE_HAFNIUM_ORE_PLACED_KEY: ResourceKey<PlacedFeature> = registerKey("middle_hafnium_ore_placed")
		val SMALL_ORE_PLACED_KEY: ResourceKey<PlacedFeature> = registerKey("small_hafnium_ore_placed")
		...
		fun boostrap(context: BootstapContext<PlacedFeature>) {
			...
			register(
				...
				configuredFeatureRegistryEntryLookup.getOrThrow(FConfiguredFeatures.HAFNIUM_ORE_KEY),
			...
			register(
				...
				FOrePlacement.modifiersWithCount(
					25,
					HeightRangePlacement.uniform(VerticalAnchor.absolute(-63), VerticalAnchor.absolute(0)),
				),
			)
			...
			register(
				...
				configuredFeatureRegistryEntryLookup.getOrThrow(FConfiguredFeatures.SMALL_HAFNIUM_ORE_KEY),
				FOrePlacement.modifiersWithCount(
					5,
		...
		fun registerKey(name: String?): ResourceKey<PlacedFeature> {
			return ResourceKey.create(Registries.PLACED_FEATURE, ResourceLocation(Fusarft.MOD_ID, name))
		}
