	...
	companion object {
		...
		fun boostrap(context: BootstapContext<ConfiguredFeature<*, *>>) {
			...
			val overworldHafniumOres: List<OreConfiguration.TargetBlockState> =
				listOf(
					OreConfiguration.target(stoneReplacables, FBlocks.HAFNIUM_ORE.defaultBlockState()),
					OreConfiguration.target(deepslateReplacables, FBlocks.DEEPSLATE_HAFNIUM_ORE.defaultBlockState()),
				)

			register(context, HAFNIUM_ORE_KEY, Feature.ORE, OreConfiguration(overworldHafniumOres, 9))
			register(context, SMALL_HAFNIUM_ORE_KEY, Feature.ORE, OreConfiguration(overworldHafniumOres, 9))
		}

		private fun registerKey(name: String): ResourceKey<ConfiguredFeature<*, *>> {
			return ResourceKey.create(Registries.CONFIGURED_FEATURE, ResourceLocation(Fusarft.MOD_ID, name))
		}
